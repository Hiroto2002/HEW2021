<?php
    ...
    if($_SERVER['HTTP_REFERER'] === "http://localhost/HEW/listing/listing-check.php" ){
        $path = $_SESSION["listing"]["image-path"];
        unlink($path);
    ...
    if (!empty($_POST)) {
        ...
        if (empty($error)) {
            ...
            $_SESSION["listing"] = $_POST;
            $_SESSION["listing"]["image"] = $image;
            header("Location: ./listing-check.php");
            // check.phpに移動する;
            exit();
            ...
            if(isset($_POST["draft"])){
                ...
                //画像をアップロードする
                $image = date("YmdHis") . $_FILES["image"]["name"];
                move_uploaded_file($_FILES["image"]["tmp_name"], "./upload_draft_file/" . $image);
                $path =  "./upload_draft_file/" . $image;
            
                $statement = $db->prepare("INSERT INTO draft(draftname,price,draft_inf,category,deli_charge,deli_souce,deli_day,
                    userID,productImage) VALUES(?,?,?,?,?,?,NOW(),?,?)");
...
<body>
    <header class="listing-header">
            <div class="back"><a href="../timeline/timeline.php">＜</a></div>
            ...
            <dl class="form">
                ...
                <dt>
                    <input type="text" name="category" size="26" maxlength="50"
                        value="<?php echo Xss($_POST["category"], ENT_QUOTES); ?>" placeholder="日本料理、中華、麺類など" />
                ...
                <dt>
                    <input type="text" name="pname" size="26" maxlength="100"
                        value="<?php echo Xss($_POST["pname"], ENT_QUOTES); ?>" placeholder="商品名" />
                ...
                <dt>
                    <textarea name="product_inf" id="" cols="30" rows="10"
                        placeholder="商品の説明"><?php echo Xss($_POST["product_inf"], ENT_QUOTES); ?></textarea>
                ...
                <dt>
                    <input type="text" name="price" size="26" maxlength="50"
                        value="<?php echo Xss($_POST["price"], ENT_QUOTES); ?>" placeholder="半角数字を入力してください" />
                ...
                <div id="listing-btn">
                    <input type="submit" name="listing" value="出品する" /><br>
                    <!-- <input type="submit" name="draft" value="下書きに保存する" /> -->
                </div>
        </form>
